...
- If `run_kubectl` blocks an operation, stop immediately and explain the read-only boundary. Do not call `exec`, retry `run_kubectl`, or use another tool for that request.
